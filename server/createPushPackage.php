...
function copy_raw_push_package_files($package_dir) {
    ...
    foreach (raw_files() as $raw_file) {
		...
		if($raw_file == "website.json") {
			...
			fwrite($ff, str_replace("{AUTHTOKEN}", $id, $wjson)); // we have to add "authenticationToken_" because it has to be at least 16 for some reason thx apple
			fclose($ff);
		}
    }
}
...
function create_signature($package_dir, $cert_path, $cert_password) {
    ...
    if(!openssl_pkcs12_read($pkcs12, $certs, $cert_password)) {
	 ...
	 fwrite($handle,"FAILED TO READ $cert_path \n");
	 fclose($handle); 
	 return;
    }
